...
func (db *DB) updateSchema(
	...
	for _, schema := range proposedDescriptionsByName {
		...
		cols, err := description.GetCollectionsBySchemaVersionID(ctx, txn, previousVersionID) // todo - this no longer needs to return an array
		if err != nil {
			return err
		}
		col := cols[0]
		...
		col.Sources = append(
			...
			&client.CollectionSource{
				SourceSchemaVersionID:      previousVersionID,
