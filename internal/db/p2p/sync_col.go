...
func (p *P2P) SyncCollections(ctx context.Context, versionIDs ...string) error {
	...
	for _, versionID := range versionIDs {
		_, err := p.syncCollection(ctx, versionID, linkSys) // todo - might as well return these :)
		if err != nil {
			return err
		}
	}
...
func (p *P2P) syncCollection(ctx context.Context, versionID string, linkSys linking.LinkSystem) (client.CollectionVersion, error) {
	...
	if len(linkBlock.Heads) > 1 {
		panic("todo - document, should never happen")
	}

	var previous immutable.Option[client.CollectionSource]
	if len(linkBlock.Heads) == 1 {
		previousID := linkBlock.Heads[0].String()
		col, err = p.syncCollection(ctx, previousID, linkSys) // todo - rework func to take cid...?
		if err != nil {
			return client.CollectionVersion{}, err
		}
	...
	for _, fieldCid := range linkBlock.Links {
		...
		if fieldDelta.RelativeID != nil {
			kind = &client.SelfKind{
				...
				Array:      false, // todo - doc
			}
		} else {
			k := *fieldDelta.ScalarKind

			switch k { // todo - mv to client + refactor (json marshal stuff)
			case uint8(client.FieldKind_BOOL_ARRAY), uint8(client.FieldKind_INT_ARRAY), uint8(client.FieldKind_FLOAT64_ARRAY),
				uint8(client.FieldKind_STRING_ARRAY), uint8(client.FieldKind_NILLABLE_BOOL_ARRAY), uint8(client.FieldKind_NILLABLE_INT_ARRAY),
				uint8(client.FieldKind_NILLABLE_FLOAT64_ARRAY), uint8(client.FieldKind_NILLABLE_STRING_ARRAY),
				uint8(client.FieldKind_FLOAT32_ARRAY), uint8(client.FieldKind_NILLABLE_FLOAT32_ARRAY):
				kind = client.ScalarArrayKind(k)
			default:
				kind = client.ScalarKind(k)
			}
		}
