...
func (p *P2P) hasAccess(ctx context.Context, pid string, c cid.Cid) bool {
	...
	if err != nil {
		if strings.Contains(err.Error(), "invalid key: \"modules\" is not a field in type Block") { // todo :)
			return true
		}
		if strings.Contains(err.Error(), "invalid key: \"lens\" is not a field in type Block") { // todo :)
			return true
		}
		if strings.Contains(err.Error(), "invalid key: \"wasmBytes\" is not a field in type Block") { // todo :)
			return true
		}
